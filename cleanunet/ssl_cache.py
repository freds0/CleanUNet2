"""
SSL Embedding Cache System

This module provides a caching mechanism for pre-extracted ssl embeddings.
Unlike the x-vector cache that extracts on-the-fly, this cache ONLY loads
pre-extracted embeddings from disk.

The embeddings must be pre-extracted using extract_ssl_embeddings.py before training.
"""
...
class SSLEmbeddingCache:
    """
    Cache system for pre-extracted SSL embeddings.
    ...
    def __init__(self, cache_dir: str, enabled: bool = True):
        ...
        if self.enabled:
            if not self.cache_dir.exists():
                print(f"[SSLEmbeddingCache] Warning: Cache directory does not exist: {self.cache_dir}")
                print(f"[SSLEmbeddingCache] Please run extract_ssl_embeddings.py first!")
                self.enabled = False
            else:
                print(f"[SSLEmbeddingCache] Cache enabled: {self.cache_dir}")
                self._stats = {
                    'hits': 0,
                    'misses': 0
                }
                ...
                if metadata_file.exists():
                    ...
                    print(f"[SSLEmbeddingCache] Loaded metadata:")
            ...
            print("[SSLEmbeddingCache] Cache disabled")
            self._stats = None
    ...
    def get(self, audio_path: str, device: str = 'cpu') -> Optional[torch.Tensor]:
        """
        Retrieve ssl embedding from cache.
        ...
        if cache_path.exists():
            try:
                ...
                print(f"[SSLEmbeddingCache] Warning: Failed to load cache for {audio_path}: {e}")
    ...
    def get_batch(self, audio_paths: list, device: str = 'cpu') -> Dict[str, Optional[torch.Tensor]]:
        """
        Retrieve multiple ssl embeddings from cache.
    ...
    def print_stats(self):
        ...
        if not stats['enabled']:
            print("[SSLEmbeddingCache] Cache is disabled")
            return

        print("\n" + "=" * 60)
        print("SSL Embedding Cache Statistics")
        print("=" * 60)
    ...
    def __repr__(self):
        return f"SSLEmbeddingCache(cache_dir={self.cache_dir}, enabled={self.enabled})"
