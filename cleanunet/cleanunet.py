...
class CleanUNet(nn.Module):
    ...
    def encode(self, noisy_audio: torch.Tensor, encoder_film=None, bottleneck_film=None):
        ...
        Returns:
            ...
            encoder_states: Dict with normalization params and skip connections
        """
        # ensure shape (B, C, L)
        ...
        if bottleneck_film is not None:
            ...
            if gamma_b.shape[-1] != x.shape[-1]:            # resample SSL-rate -> bottleneck-rate
    ...
    def decode(self, latent: torch.Tensor, encoder_states: dict) -> torch.Tensor:
        """
        Decode latent representation to enhanced audio.
        Used for X-Vector integration in two-stage training.
        ...
        Returns:
            enhanced_audio: Enhanced audio (B, 1, L)
        """
        x = latent
        skip_connections = encoder_states['skip_connections']
        std = encoder_states['std']
        L = encoder_states['original_length']
    ...
    def forward(self, noisy_audio: torch.Tensor) -> torch.Tensor:
        ...
        """
        latent, encoder_states = self.encode(noisy_audio)
        return self.decode(latent, encoder_states)
