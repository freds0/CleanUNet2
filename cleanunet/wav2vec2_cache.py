"""
Wav2Vec2 Embedding Cache System

This module provides a caching mechanism for pre-extracted wav2vec2 embeddings.
Unlike the x-vector cache that extracts on-the-fly, this cache ONLY loads
pre-extracted embeddings from disk.

The embeddings must be pre-extracted using extract_wav2vec2_embeddings.py before training.
"""
...
class Wav2Vec2Cache:
    """
    Cache system for pre-extracted Wav2Vec2 embeddings.
    ...
    def __init__(self, cache_dir: str, enabled: bool = True):
        ...
        if self.enabled:
            if not self.cache_dir.exists():
                print(f"[Wav2Vec2Cache] Warning: Cache directory does not exist: {self.cache_dir}")
                print(f"[Wav2Vec2Cache] Please run extract_wav2vec2_embeddings.py first!")
                self.enabled = False
            else:
                print(f"[Wav2Vec2Cache] Cache enabled: {self.cache_dir}")
                self._stats = {
                    'hits': 0,
                    'misses': 0
                }
                ...
                if metadata_file.exists():
                    ...
                    print(f"[Wav2Vec2Cache] Loaded metadata:")
            ...
            print("[Wav2Vec2Cache] Cache disabled")
            self._stats = None
    ...
    def get(self, audio_path: str, device: str = 'cpu') -> Optional[torch.Tensor]:
        """
        Retrieve wav2vec2 embedding from cache.
        ...
        if cache_path.exists():
            try:
                ...
                print(f"[Wav2Vec2Cache] Warning: Failed to load cache for {audio_path}: {e}")
    ...
    def get_batch(self, audio_paths: list, device: str = 'cpu') -> Dict[str, Optional[torch.Tensor]]:
        """
        Retrieve multiple wav2vec2 embeddings from cache.
    ...
    def print_stats(self):
        ...
        if not stats['enabled']:
            print("[Wav2Vec2Cache] Cache is disabled")
            return

        print("\n" + "=" * 60)
        print("Wav2Vec2 Embedding Cache Statistics")
        print("=" * 60)
    ...
    def __repr__(self):
        return f"Wav2Vec2Cache(cache_dir={self.cache_dir}, enabled={self.enabled})"
