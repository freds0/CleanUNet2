...
Uses microsoft/wavlm-large for self-supervised speech embeddings
"""
...
def select_layer_indices(num_selected, total_states):
    ...
    (endpoints included). For WavLM-Large there are 25 hidden states (embedding
    output + 24 transformer layers), so:
...
class WavLMExtractor(nn.Module):
    """
    WavLM embedding extractor using microsoft/wavlm-large.
    ...
    def __init__(self, model_name="microsoft/wavlm-large", device='cpu', layer=12,
        ...
        Args:
            model_name (str): HuggingFace model name (default: microsoft/wavlm-large)
        ...
        print(f"[WavLMExtractor] Selected layers (of {self.total_states}): {self.selected_layers}")
        ...
        if self.use_weighted_layers:
            ...
            print(f"[WavLMExtractor] Weighted-sum over {len(self.selected_layers)} selected layers "
    ...
    def extract_embeddings(self, waveform, sample_rate=16000, return_mean=True):
        ...
        # Run the frozen WavLM backbone under no_grad.
        with torch.no_grad():
            outputs = self.model(
                waveform,
    ...
    def extract_selected_layers(self, waveform, sample_rate=16000):
        ...
        can then be applied cheaply at training time (without re-running WavLM).
