"""
PyTorch Lightning module for CleanUNet2 Stage-1 training with SSL Embeddings (WavLM).

Stage-1: Training with WavLM embeddings injected into the latent space. When
`wavlm_use_weighted_layers` is enabled, the embedding is a learnable softmax-weighted
sum over ALL WavLM layers (the layer weights are trained jointly here).
Fused latent vectors are saved for Stage-2 training.
"""
...
class CleanUNet2SSLEmbeddingsStage1Module(pl.LightningModule):
    """
    Lightning module for Stage-1 training with SSL Embeddings (WavLM).

    Trains the model using WavLM embeddings extracted from clean audio.
    ...
    def __init__(self, config):
        ...
        print("=" * 80)
        print("STAGE-1: Training with SSL Embeddings (WavLM)")
        print("=" * 80)
        ...
        if sr < 8000:
            ...
            print(f"[Stage-1] WARNING: Sample rate {sr} Hz; PESQ only supports 8k/16k. Will downsample.")
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        for key, value in state_dict.items():
            if any(key.startswith(ignore_key) for ignore_key in keys_to_ignore):
    ...
    def configure_optimizers(self):
        ...
        # The WavLM backbone is frozen; everything else (including the learnable
        # per-layer softmax weights and the integration block) is trainable.
