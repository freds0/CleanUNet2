...
class CleanUNet2SpeakerEmbeddingsStage2Module(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # ===== Per-file Stage-1 latent caches (distillation targets) =====
        # Both caches are per-file <md5(clean_path)>.pt, generated by
        # generate_train_latents.py from a trained Stage-1 checkpoint. Training and
        # validation use the same path-keyed lookup; alignment relies on the matching
        ...
        if not train_dir:
            raise ValueError(
                ...
                "`generate_train_latents.py --split train` and set 'train_latents_dir'."
            )
        ...
        # VAL latents live in the SAME directory as the train cache (one per-file <md5>.pt
        # cache; train and val clean paths are disjoint, so keys never collide). Generate
        # both splits into train_latents_dir with generate_train_latents.py. The validation
        # latent metric is computed the same path-keyed way; it stays 0 for any val sample
        # whose latent was not generated (OPTIONAL: never aborts).
        self.val_latents_dir = self.train_latents_dir
        self.val_latent_keys = self.train_latent_keys
        self._val_latent_mem = self._train_latent_mem
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # ===== Compute Latent Replication Loss (path-keyed val cache; 0 if absent) =====
        loss_latent = self._cached_latent_loss(
            predicted_latent, clean_audio_paths,
