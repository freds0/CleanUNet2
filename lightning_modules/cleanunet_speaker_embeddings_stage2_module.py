...
class CleanUNet2SpeakerEmbeddingsStage2Module(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # ===== Per-file TRAIN latent cache (used by the training-time latent loss) =====
        # Generated by generate_train_latents.py from a trained Stage-1 checkpoint, keyed
        ...
        self.train_latents_dir = Path(train_dir) if train_dir else None
        self._train_latent_mem = {}
        if self.train_latents_dir is not None and self.train_latents_dir.exists():
            self.train_latent_keys = {p.stem for p in self.train_latents_dir.glob('*.pt')}
            ...
            self.train_latent_keys = set()
            print(f"[Stage-2] No train latent cache (train_latents_dir={self.train_latents_dir}); "
                  f"training-time latent loss DISABLED (loss_latent=0).")
    ...
    def _load_stored_latents(self):
        """Load all stored VAL latents (val_batch_*.pt) from Stage-1.

        The val cache is optional (see __init__), so a directory that exists but
        holds no val_batch_*.pt returns {} and only disables the validation latent
        metric — it must not abort Stage-2 training.
        """
