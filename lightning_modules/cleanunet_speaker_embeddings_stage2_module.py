"""
PyTorch Lightning module for CleanUNet2 Stage-2 training with Speaker Embeddings.

Stage-2: Training without speaker embedding extraction, replicating latent vectors from Stage-1.
...
class CleanUNet2SpeakerEmbeddingsStage2Module(pl.LightningModule):
    """
    Lightning module for Stage-2 training with Speaker Embeddings (X-Vector).

    Trains the model to replicate Stage-1's fused latent vectors using
    only the noisy audio (no speaker embedding extractor).
    """
    ...
    def __init__(self, config):
        ...
        print("=" * 80)
        print("STAGE-2: Replicating Latents (Speaker Embeddings - Inference Mode)")
        print("=" * 80)
        ...
        self.model = CleanUNet2WithSpeakerEmbeddings(**model_args)
        ...
        # ===== Per-file Stage-1 latent cache (distillation targets) =====
        # ONE per-file <md5(clean_path)>.pt cache in 'latents_dir', generated by
        # generate_train_latents.py from a trained Stage-1 checkpoint. Training and
        # validation share it (train/val clean paths are disjoint, so keys never collide);
        # alignment relies on the matching deterministic crop (applied to train AND val)
        # so each sample lines up with its cached fused-latent target.
        #
        # REQUIRED: Stage-2 trains ONLY if the cache exists.
        latents_dir = config.get('latents_dir')
        if not latents_dir:
            raise ValueError(
                "[Stage-2] 'latents_dir' is not set in the config. Stage-2 distillation "
                "requires the Stage-1 latents. Generate them with "
                "generate_train_latents.py and set 'latents_dir'."
            )
        self.latents_dir, self.latent_keys = self._load_latent_cache(latents_dir)
        self._latent_mem = {}
        if not self.latent_keys:
            raise FileNotFoundError(
                f"[Stage-2] No latents found in '{self.latents_dir}'. Stage-2 will not "
                f"train without them. Generate first (both splits into the same dir):\n"
                f"  python generate_train_latents.py --split train --config <stage1-config> "
                f"--checkpoint <stage1.ckpt> --output-dir {self.latents_dir}\n"
                f"  python generate_train_latents.py --split val   --config <stage1-config> "
                f"--checkpoint <stage1.ckpt> --output-dir {self.latents_dir}"
            )
        print(f"[Stage-2] Latent cache: {len(self.latent_keys)} files "
              f"in {self.latents_dir} (latent loss ENABLED)")
        # TRAIN must align (abort otherwise); VAL only warns (metric stays 0 if not generated).
        self._assert_latents_aligned(config, 'train_list_path', self.latents_dir,
                                     self.latent_keys, required=True)
        self._assert_latents_aligned(config, 'val_list_path', self.latents_dir,
                                     self.latent_keys, required=False)
    ...
    def _assert_latents_aligned(self, config, list_key, latents_dir, keys,
        ...
        if ratio < min_ratio:
            msg = (f"[Stage-2] Latents MISALIGNED with {list_key}: only {hit}/{len(pairs)} "
                   ...
                   f"data_dir/{list_key} mismatch. Regenerate with generate_train_latents.py "
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Forward without speaker embeddings
        ...
        loss_latent = self._cached_latent_loss(
            ...
            self.latents_dir, self.latent_keys, self._latent_mem)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # ===== Compute Latent Replication Loss (shared path-keyed cache; 0 if absent) =====
        loss_latent = self._cached_latent_loss(
            predicted_latent, clean_audio_paths,
            self.latents_dir, self.latent_keys, self._latent_mem)
    ...
    def configure_optimizers(self):
        ...
        # All parameters are trainable in Stage-2 (no frozen extractor)
